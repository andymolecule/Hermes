...
export interface AuthoringCompilerFixtureCase {
...
export interface AuthoringCompilerFixtureExecutionOverrides {
...
export function buildAuthoringCompilerFixtureExecutionOverrides(
  benchmarkCase: AuthoringCompilerFixtureCase,
): AuthoringCompilerFixtureExecutionOverrides | null {
...
export function loadAuthoringCompilerFixtureCases() {
  return fs
    ...
    .sort()
    .flatMap((benchmarkId) => {
      const benchmarkDir = path.join(FIXTURE_ROOT, benchmarkId);
      const benchmark = JSON.parse(
        fs.readFileSync(path.join(benchmarkDir, "fixture.json"), "utf8"),
      ) as BenchmarkJson;
      ...
      return benchmark.prompt_variants.map((variant) => {
        ...
        return {
          ...
          uploadedArtifacts,
          artifactTextByUri,
        } satisfies AuthoringCompilerFixtureCase;
      });
    });
}

export function buildAuthoringCompilerFixtureDependencies(
  benchmarkCase: AuthoringCompilerFixtureCase,
) {
  ...
  return {
    fetchImpl: async (url: string | URL | Request) => {
      ...
      throw new Error(
        `Unexpected fetch in authoring compiler fixture test: ${requestUrl}`,
      );
    },
