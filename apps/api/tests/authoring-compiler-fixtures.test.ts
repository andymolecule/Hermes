...
import {
  buildAuthoringCompilerFixtureDependencies,
  buildAuthoringCompilerFixtureExecutionOverrides,
  loadAuthoringCompilerFixtureCases,
} from "./authoring-compiler-fixtures.js";

const benchmarkCases = loadAuthoringCompilerFixtureCases();

for (const benchmarkCase of benchmarkCases) {
  const benchmarkLabel = `${benchmarkCase.benchmark.id}/${benchmarkCase.variantId}`;

  test(`authoring compiler fixture ${benchmarkLabel} matches compile invariants`, async () => {
    const promptOnlyResult = await compileAuthoringSessionOutcome(
      ...
      buildAuthoringCompilerFixtureDependencies(benchmarkCase),
    );
    ...
    assert.equal(
      variant.acceptable_compile_states.includes("awaiting_input"),
      true,
    );
    ...
    const overrides =
      buildAuthoringCompilerFixtureExecutionOverrides(benchmarkCase);
    if (!overrides) {
      throw new Error(
