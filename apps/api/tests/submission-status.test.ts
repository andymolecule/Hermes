...
test("buildSubmissionStatusPayload returns intent_created when no tracked on-chain submission exists", () => {
  ...
  assert.equal(payload.submission, null);
  assert.equal(payload.statusDetail, null);
});

test("buildSubmissionStatusPayload surfaces blocked scoring detail from the queued job reason", () => {
  const payload = buildSubmissionStatusPayload({
    submission: {
      id: "22222222-2222-4222-8222-222222222222",
      challenge_id: "11111111-1111-4111-8111-111111111111",
    ...
    scoreJob: {
      ...
      attempts: 1,
      max_attempts: 3,
      last_error: "challenge_scoring_not_started",
  ...
  assert.equal(payload.phase, "scoring_queued");
  assert.equal(
    payload.statusDetail,
    "Deadline passed, but scoring has not started on-chain yet.",
  );
});

test("buildSubmissionStatusPayload keeps open-challenge queue detail generic", () => {
  const payload = buildSubmissionStatusPayload({
    submission: {
      id: "22222222-2222-4222-8222-222222222222",
      challenge_id: "11111111-1111-4111-8111-111111111111",
    ...
    scoreJob: {
      ...
      attempts: 1,
      max_attempts: 3,
      last_error: "challenge_not_in_scoring",
      next_attempt_at: null,
      locked_at: null,
    } as never,
    unmatchedSubmission: null,
  ...
  assert.equal(
    payload.statusDetail,
    "Challenge is not in scoring yet. Official scoring starts after the deadline and persisted startScoring().",
  );
});
