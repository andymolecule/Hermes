import { applyConfigToEnv, loadCliConfig, requireConfigValues } from "./config-store";
...
export function resolvePrivateKeyFromArg(keyArg?: string): string | undefined {
  ...
  if (!keyArg.startsWith("env:")) {
    throw new Error("Private key must be provided via env:VAR_NAME (never pass raw keys as args).");
  }
...
export function prepareHermesEnv(requiredKeys: (keyof ReturnType<typeof loadCliConfig>)[]) {
