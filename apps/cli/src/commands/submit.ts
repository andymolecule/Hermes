...
import { createSupabaseClient, getChallengeById, setSubmissionResultCid } from "@hermes/db";
import { pinFile } from "@hermes/ipfs";
import { getPublicClient, getWalletClient, submitChallengeResult } from "@hermes/chain";
...
const HermesChallengeAbi = HermesChallengeAbiJson as unknown as readonly unknown[];
...
export function buildSubmitCommand() {
  const cmd = new Command("submit")
    ...
    .action(async (file: string, opts: { challenge: string; dryRun: boolean; key?: string; format: string }) => {
      ...
      const resultCid = await pinFile(path.resolve(process.cwd(), file), path.basename(file));
      pinSpinner.succeed(`Pinned result: ${resultCid}`);

      if (opts.dryRun) {
        const output = { challengeId: opts.challenge, resultCid, dryRun: true };
        if (opts.format === "json") {
          printJson(output);
        } else {
      ...
      const challenge = (await getChallengeById(db, opts.challenge)) as ChallengeRecord;
      ...
      if (typeof submissionId === "bigint") {
        await setSubmissionResultCid(db, challenge.id, Number(submissionId), resultCid);
      }
      ...
      const output = {
        submissionId: typeof submissionId === "bigint" ? Number(submissionId) : submissionId,
        resultCid,
        txHash,
      ...
      if (opts.format === "json") {
        ...
        if (submissionCount !== null) {
          printWarning(`Submission ${submissionCount}/${challenge.max_submissions_per_wallet} used`);
        }
      }
    });
