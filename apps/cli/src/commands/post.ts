...
import { challengeSpecSchema, type ChallengeSpecOutput } from "@hermes/common";
...
export function buildPostCommand() {
  const cmd = new Command("post")
    ...
    .option("--dry-run", "Validate and pin, but skip on-chain transactions", false)
    .option("--key <ref>", "Private key reference, e.g. env:HERMES_PRIVATE_KEY")
    .option("--format <format>", "table or json", "table")
    .action(async (file: string, opts: { deposit?: string; dryRun: boolean; key?: string; format: string }) => {
      const config = loadCliConfig();
      applyConfigToEnv(config);
      requireConfigValues(config, ["rpc_url", "factory_address", "usdc_address", "pinata_jwt"]);
      ...
      if (!validation.success) {
        throw new Error(`Invalid challenge spec:\n${formatZodError(validation.error)}`);
      }
      const spec = validation.data;

      if (!(spec.reward.distribution in distributionMap)) {
        throw new Error(`Unsupported reward distribution: ${spec.reward.distribution}`);
      }
      ...
      if (opts.dryRun) {
        ...
        if (opts.format === "json") {
          ...
          printSuccess("Dry run complete. No on-chain transactions were sent.");
        }
        return;
      }
      ...
      allowanceSpinner.succeed(`Allowance: ${formatUnits(allowanceAmount, 6)} USDC`);
      ...
      const txHash = await createChallenge({
        ...
        labTba: (spec.lab_tba ?? "0x0000000000000000000000000000000000000000") as `0x${string}`,
      });
      ...
      const output = {
        id: typeof challengeId === "bigint" ? Number(challengeId) : challengeId,
