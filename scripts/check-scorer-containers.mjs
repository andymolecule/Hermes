...
const containersDir = path.join(rootDir, "containers");
...
if (!fs.existsSync(containersDir)) {
  fail("containers/ directory not found.");
}

const containerDirs = fs
  .readdirSync(containersDir, { withFileTypes: true })
  .filter((entry) => entry.isDirectory())
  .map((entry) => path.join(containersDir, entry.name))
  .sort();
