...
All design tokens are defined in `apps/web/src/app/globals.css` using Tailwind 4's `@theme` block. That file is the canonical, executable source of truth for colors, fonts, spacing, and motion.

Semantic tokens (surfaces, text colors, borders, glass effects) are CSS custom properties in `:root` with dark-mode overrides in `[data-theme="dark"]`.
