...
- **worker_runtime_control** — Active scoring-runtime control row. Once hosted readiness passes, the API keeps the active runtime version in sync here. Score-job claims are fenced against it so stale workers cannot keep claiming jobs if a Fly rolling deploy, restarted Machine, or manual recovery briefly leaves mixed runtime versions alive.
