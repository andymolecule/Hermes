...
- **worker_runtime_control** — Active scoring-runtime control row. While the runtime schema is healthy and the public API release matches the running API runtime, the API keeps the active runtime version in sync here. Score-job claims are fenced against it so older workers cannot keep claiming jobs after a deploy, and failed unpromoted API candidates cannot seize the fence early.
