...
- if a session is already `ready`, wallet-funded publish may be re-prepared
  safely, but `confirm-publish` must still validate the live `createChallenge`
  transaction against the active factory because deadline and contract-limit
  reverts are chain-state dependent rather than authoring-state dependent
- if that live wallet transaction or the later confirmation path fails,
